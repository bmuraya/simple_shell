...
void print_syntax_error(shell_bd *db_shell, char *input, int i, int bool)
{
	...
	msg2 = ": Syntax error: \"";
	msg3 = "\" unexpected\n";
	counter = aux_itoa(db_shell->counter);
	length = _strlen(db_shell->av[0]) + _strlen(counter);
...
int check_syntax_error(shell_bd *db_shell, char *input)
{
	...
	if (f_char == -1)
	{
		print_syntax_error(db_shell, input, begin, 0);
		return (1);
	}
	...
	if (i != 0)
	{
		print_syntax_error(db_shell, input, begin + i, 1);
		return (1);
	}
